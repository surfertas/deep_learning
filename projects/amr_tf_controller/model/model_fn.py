...
def build_model(is_training, inputs, params):
    ...
    with tf.variable_scope('fc_2'):
        logits = tf.layers.dense(out, params.num_features)
...
def build_model_0000(is_training, inputs, params):
    ...
    Args:
        is_training: (bool) whether we are training or not
        inputs: (dict) contains the inputs of the graph (features, targets...)
                this can be `tf.placeholder` or outputs of `tf.data`
        params: (Params) hyperparameters
...
def model_fn(mode, inputs, params, reuse=False):
    ...
    #print("LABELS", targets.get_shape())
    ...
    loss = tf.losses.mean_squared_error(labels=targets, predictions=logits)
    ...
    with tf.variable_scope("metrics"):
        ...
        metrics = {
            'mse': tf.metrics.mean_squared_error(labels=targets, predictions=logits),
